...
Esta versão combina múltiplas etapas de pré-processamento e heurísticas
morfológicas para detectar insetos mesmo em imagens ruidosas ou com
iluminação irregular.

Sistemas integrados nesta versão:
 - Pré-processamento com CLAHE, redução de ruído e suavização leve.
 - Três faixas HSV independentes para capturar adultos, ninfas e regiões
   que servem como fallback quando a imagem foge do padrão.
 - Extração de candidatos baseada em máscara de cor + bordas (Canny),
   reduzindo falsos positivos em texturas da folha.
 - Avaliação dos candidatos através de um score multi-critério
   (área, circularidade, aspecto, solidez, preenchimento da máscara etc.).
 - Non-Max Suppression simples para remover caixas sobrepostas.
 - Geração de arquivos YOLO (.txt) com coordenadas normalizadas.
...
# Limiar mínimo para aceitar uma detecção final. Reduzir esse valor fará
# com que mais insetos sejam detectados, porém aumenta falsos positivos.
DEFAULT_CONFIDENCE = 0.55

# Número de imagens por split que serão renderizadas como exemplo
VIS_SAMPLES_PER_SPLIT = 12


class WhiteflyAutoLabelerV3:
    """
    Classe principal responsável por todo o pipeline de geração automática
    de labels do dataset. Ela realiza desde o pré-processamento até a escrita
    final dos arquivos no formato YOLO.
    """
    ...
    def __init__(self, confidence_threshold: float = DEFAULT_CONFIDENCE, debug: bool = False):
        ...
        # Faixas HSV calibradas com base no seu dataset real
        # Cada faixa representa um tipo de inseto (ou fallback)
        # e define limites de área e peso relativo no score final.
        self.stages = {
            'adulto': {
        ...
        # Máscara aproximada da folha usada só como referência contextual
    ...
    def _preprocess(self, img: np.ndarray) -> Dict[str, np.ndarray]:
        """
        Aplica uma cadeia de pré-processamento para estabilizar a detecção:
        - Resize leve para imagens pequenas (melhora consistência).
        - CLAHE para realçar contrastes locais.
        - Denoise e blur suave para reduzir ruído cromático.
        - Conversão para HSV, Gray e detecção de bordas (Canny).
        """

        h, w = img.shape[:2]
        scale = 1.0

        # Amplia imagens pequenas para melhorar o comportamento das máscaras
        if max(h, w) < 600:
            scale = 1.5
    ...
    def _stage_mask(self, hsv: np.ndarray, stage_name: str) -> np.ndarray:
        """
        Gera a máscara binária para um dos três estágios (adulto, ninfa ou geral)
        aplicando também operações morfológicas para remover ruído fino.
        """
    ...
    def _extract_candidates(self, pre: Dict[str, np.ndarray]) -> List[Dict]:
        """
        Extrai regiões suspeitas de serem insetos combinando:
        - Máscaras HSV por estágio.
        - Interseção com bordas Canny para descartar manchas do fundo.
        - Fusão final das duas informações.
        A partir disso, contornos válidos são filtrados por tamanho e
        ...
        for stage_name in self.stages.keys():
            ...
            for cnt in cnts:
                ...
                # Mantém apenas regiões na faixa esperada para insetos
    ...
    def _score_candidate(self, cand: Dict, pre: Dict[str, np.ndarray]) -> float:
        """
        Calcula um score final combinando:
         - Área ideal esperada para o estágio.
         - Solidez (insetos são sempre regiões compactas).
         - Circularidade aproximada.
         - Presença dentro da máscara e fusão máscara+borda.
         - Aspect ratio característico do corpo.
         - Momento de Hu para forma compacta.
         - Peso relativo do estágio (adulto, ninfa, geral).
        """
    ...
    def _nms(self, detections: List[Dict], iou_thres: float = 0.35) -> List[Dict]:
        """
        Remove caixas sobrepostas mantendo somente as de maior confiança.
        Implementação simples baseada em IoU entre bounding boxes.
        """
        if not detections:
            return []
    ...
    def detect_on_image(self, image_path: Path) -> List[Dict]:
        """
        Executa todo o pipeline de detecção em uma imagem:
         1. Pré-processamento
         2. Extração de candidatos
         3. Cálculo do score
         4. Non-Max Suppression
         5. Filtro final pelo limiar de confiança
        """
        ...
        for c in candidates:
            ...
            # Mantém primeiros candidatos com limiar reduzido
        ...
        # Reverte escala caso imagem tenha sido ampliada no pré-processamento
    ...
    def to_yolo_lines(self, dets: List[Dict], img_w: int, img_h: int) -> List[str]:
        """
        Converte detecções para o formato YOLO:
        classe cx cy w h (tudo normalizado entre 0 e 1)
        Neste dataset, sempre classe 0 (mosca-branca).
        """
        lines = []
        for d in dets:
            x,y,w,h = d['bbox']
    ...
    def visualize(self, src_img_path: Path, detections: List[Dict], out_path: Path):
        """
        Salva uma imagem com bounding boxes marcadas para inspeção manual.
        Cores variam conforme a confiança da detecção.
        """
    ...
    # ----------------------------------------------------------------------
    # Processa todo o dataset em train/val/test
    # ----------------------------------------------------------------------
    def process_dataset(self, dataset_root: Path, visualize_samples: int = VIS_SAMPLES_PER_SPLIT):
        """
        Percorre todos os splits do dataset (train/val/test) e gera:
         - labels YOLO em /labels/<split>/
         - amostras de visualização
         - relatório numérico das detecções
        Também cria automaticamente um backup de labels existentes.
        """
        ...
        # Criação de backup caso já exista pasta de labels
        ...
        for split in ['train','val','test']:
            ...
            for img_path in tqdm(imgs, desc=f"{split}"):
                ...
                # Visualização de algumas amostras por split
